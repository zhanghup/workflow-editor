<template>
  <Panel direct="right" title="属性设置" width="450px">
    <div class="z-form">
      <el-form v-if="form" style="width: 100%;" :model="form">
        <el-form-item label="标签名称">
          <el-input v-model="form.label" @update:model-value="changeLabel"/>
        </el-form-item>
        <el-form-item label="占位提示">
          <el-input v-model="form.placeholder" @change="changeForm"/>
        </el-form-item>
        <el-form-item v-if="[CompType.Radio,CompType.Checkbox,CompType.Switch].indexOf(form.type) === -1" label="是否可清空">
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    return {
      ...
      addOption() {
        form.value.options?.push({label: "选项", value: "0"})
      }
    }
  }
})
